...
class Requester extends Monitorable(Configurable(Component)) {
  ...
  socketSend(...args) {
    const {
      socks
    } = this.sock; // Enqueue if no socks connected yet
    ...
    if (!possibleSocks.length) return this.sock.enqueue(args); // Balances out node availability
