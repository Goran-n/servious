...
module.exports = class Component extends EventEmitter {
  constructor(advertisement, explorerOptions = {}) {
    ...
    this.explorerOptions = { ..._explorer.default.defaults,
      ...explorerOptions
    };
  ...
  startExplorer() {
    ...
    this.explorer.on('added', item => {
      if (item.advertisement.axon_type != this.oppo || item.advertisement.key != this.advertisement.key || this.advertisement.namespace != item.advertisement.namespace) return;
      this.onAdded(item);
